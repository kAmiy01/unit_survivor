...
function openModal(id) {
    ...
    if (id === null) {
        ...
        if (course.absentLimit !== undefined && course.absentLimit !== null) {
        document.getElementById('edit-absentLimit').value = course.absentLimit;
        } else {
            document.getElementById('edit-absentLimit').value = 5; // デフォルト値
        }

        // 遅刻換算（0も数値として扱う）
        if (course.lateToAbsentRatio !== undefined && course.lateToAbsentRatio !== null) {
            document.getElementById('edit-lateToAbsentRatio').value = course.lateToAbsentRatio;
        } else {
            document.getElementById('edit-lateToAbsentRatio').value = 3; // デフォルト値
        }
        // Reset Sliders
