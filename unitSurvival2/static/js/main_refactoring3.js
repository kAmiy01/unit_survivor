...
//色設定
const STATUS_CONFIG = {
    FAILED:   { color: 'var(--matte-600)', borderColor: 'var(--matte-600)', textClass: 'text-muted glitch-text', cardClass: 'status-failed',   label: '落単',   badgeClass: 'badge-gray',   btnClass: 'btn-base' },
    CRITICAL: { color: 'var(--led-red)',   borderColor: 'var(--led-red)',   textClass: 'text-red text-glow',      cardClass: 'status-critical', label: '危険',   badgeClass: 'badge-red',    btnClass: 'btn-grad-red' },
    WARNING:  { color: 'var(--led-yellow)',borderColor: 'var(--led-yellow)',textClass: 'text-yellow text-glow',   cardClass: 'status-warning',  label: '注意',   badgeClass: 'badge-yellow', btnClass: 'btn-grad-yellow' },
    SAFE:     { color: 'var(--led-blue)',  borderColor: 'var(--led-blue)',  textClass: 'text-blue',               cardClass: 'status-active',   label: '履修中', badgeClass: 'badge-blue',   btnClass: 'btn-base' }
};
...
//色設定用関数
function getCourseStatus(remaining) {
    if (remaining < 0) return STATUS_CONFIG.FAILED;
    if (remaining <= 1) return STATUS_CONFIG.CRITICAL;
    if (remaining === 2) return STATUS_CONFIG.WARNING;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    //! attachEventListenersが冗長であったため、以下のコードを追加して削除した。
    courseList.addEventListener('click', (e) => {
        // 1. クリックされた要素が .count-btn (またはその子要素) かどうか判定
        const targetBtn = e.target.closest('.count-btn');
        
        // .count-btn 以外がクリックされたら何もしないで終了
        if (!targetBtn) return;

        // 2. データ属性を取得
        const id = parseInt(targetBtn.dataset.id);
        const type = targetBtn.dataset.type;   // 'absent' or 'late'
        const delta = parseInt(targetBtn.dataset.delta); // 1 or -1

        // 3. 確認メッセージの作成
        let msg = '';
        if (delta > 0) {
            msg = (type === 'absent' ? '欠席を記録しますか？' : '遅刻を記録しますか？');
        } else {
            msg = '記録を取り消しますか？';
        }
        ...
        showConfirm(msg, () => {
            ...
            // データの更新 (Optimistic Update)
            if (type === 'absent') {
                course.absentCount = Math.max(0, course.absentCount + delta);
            } else {
                course.lateCount = Math.max(0, course.lateCount + delta);
            }
...
async function apiRequest(endpoint, method = 'GET', data = null) {
    ...
    if (!res.ok) {
        ...
        // エラーでも処理を止めない場合はここで throw せず return null などにする
    }

    // レスポンスが空の場合の対策（Reset時など）
...
async function saveTerms() {
    ...
    await apiRequest('/api/terms', 'POST', inputs);
...
async function apiUpdateCourse(id, data) {
    await apiRequest(`/api/courses/${id}`, 'PUT', data);
    fetchCourses();
}
...
function renderCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        dayCourses.forEach(c => {
            const rem = calculateRemaining(c); //! 遅刻換算がないときの処理もできるように変更
            const status = getCourseStatus(rem);
            
            dotsHtml += `<div class="calendar-dot" style="background-color: ${status.color}; box-shadow: 0 0 4px ${status.color};"></div>`;
        });
        dotsHtml += '</div>';
...
function showDailyDetail(day, dayCourses) {
    ...
    if (dayCourses.length === 0) {
        ...
        dayCourses.forEach(c => {
            const rem = calculateRemaining(c); //! 遅刻換算処理変更点

            const status = getCourseStatus(rem);

            list.innerHTML += `
                <div class="detail-item" style="border-left-color: ${status.borderColor};">
                    <div>
                    ...
                    <div class="text-center">
                        ...
                        <div class="${status.textClass}" style="font-family:var(--font-mono); font-weight:bold;">${rem < 0 ? '落単' : rem}</div>
                    </div>
                </div>
            `;
        });
...
function calculateRemaining(course) {
    if(course.lateToAbsentRatio === 0) { //! 遅刻換算が必要ないときの処理を追加
        return course.absentLimit - course.absentCount;
    }
...
function renderDashboard() {
    ...
    courses.forEach(course => {
        ...
        const status = getCourseStatus(remaining);
        const mainDisplayText = remaining < 0 ? GAME_OVER_TEXT : remaining;
        //! 遅刻換算がないときの条件を追加 ${course.lateToAbsentRatio === 0 ? '<span>遅刻換算なし</span>' : `<span>${course.lateToAbsentRatio}</span> 遅刻 = 1 欠席`}</div>
        ...
        card.innerHTML = `
            ...
            <div class="card-display">
                ...
                <div class="display-sub">
                    ...
                    <div class="display-sub-val">
                    ${course.lateToAbsentRatio === 0 ? '<span>遅刻換算なし</span>' : `<span>${course.lateToAbsentRatio}</span> 遅刻 = 1 欠席`}</div>
...
function attachSelectListeners() {
    document.querySelectorAll('.select-updater').forEach(select => {
        select.addEventListener('change', (e) => {
            const id = parseInt(e.target.dataset.id);
            const type = e.target.dataset.type; // 'absent' or 'late'
            ...
            // データの更新
            if (type === 'absent') {
                course.absentCount = newValue;
            } else {
                course.lateCount = newValue;
            }
...
function openModal(id) {
    ...
    if (id === null) {
        ...
        if (course.day) {
            ...
            slots.forEach(slot => {
                const dayMatch = slot.match(/([月火水木金土日])/);
...
function saveCourse() {
    ...
    const rawAbsentLimit = parseInt(document.getElementById('edit-absentLimit').value);
    const rawLateRatio = parseInt(document.getElementById('edit-lateToAbsentRatio').value);
    ...
    const data = {
        ...
        absentLimit: (!isNaN(rawAbsentLimit) && rawAbsentLimit >= 0 && rawAbsentLimit <= 5) ? rawAbsentLimit : 5,
        lateToAbsentRatio: (!isNaN(rawLateRatio) && rawLateRatio >= 0 && rawLateRatio <= 5) ? rawLateRatio : 3,
